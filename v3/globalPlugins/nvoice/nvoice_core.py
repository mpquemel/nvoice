...
class NVoiceCore:
    ...
    def clear_history(self) -> None:
        self.history.clear()
        ui.message("Histórico limpo. Memória zerada.")
    ...
    def toggle_recording(self, use_context: bool = False) -> None:
        if not self.is_recording:
            ...
            msg = "Ouvindo com contexto..." if use_context else "Ouvindo..."
            ui.message(msg)
    ...
    def _process_pipeline(self, audio_file: str, context_text: str) -> None:
        try:
            ...
            final_prompt = text
            if context_text:
                final_prompt = f"O usuário está com o foco neste texto na tela:\n'''{context_text}'''\n\nPergunta do usuário: {text}"
            ...
            logHandler.log.error(f"Pipeline Erro: {e}")
            queueHandler.queueFunction(queueHandler.eventQueue, ui.message, "Erro interno no processamento.")
        finally:
            if os.path.exists(audio_file):
                os.remove(audio_file)

    def _transcribe_local(self, audio_path: str) -> str:
        """Transcreve usando o Whisper CLI instalado globalmente no Windows do usuário."""
        ...
        # Comando: whisper nvoice_capture.wav --model base --language pt --output_format txt --output_dir %TEMP%
        command = [
            "whisper",
            audio_path,
            "--model", model,
        ...
        # 0x08000000 é CREATE_NO_WINDOW no Windows. 
        # Impede que a tela preta do DOS pisque e roube o foco do NVDA!
        CREATE_NO_WINDOW = 0x08000000
        
        try:
            # Roda o comando no sistema operacional invisivelmente
            ...
            if result.returncode != 0:
                ...
                queueHandler.queueFunction(queueHandler.eventQueue, ui.message, "Erro no Whisper Local.")
                return ""
                
            # O Whisper gera um arquivo txt com o mesmo nome do wav original
            ...
            if os.path.exists(txt_file):
                with open(txt_file, "r", encoding="utf-8") as f:
                    text = f.read().strip()
                os.remove(txt_file) # Limpeza!
                return text
            return ""
            
        except FileNotFoundError:
            # Cai aqui se o Windows disser: "O comando 'whisper' não existe"
            msg = "Aviso: O comando Whisper não está instalado no seu computador. Mude o STT para Groq ou instale o Whisper."
    ...
    def _transcribe_groq(self, audio_path: str) -> str:
        ...
        if not api_key:
            queueHandler.queueFunction(queueHandler.eventQueue, ui.message, "Falta API Key do Groq para STT.")
            return ""
        ...
        try:
            ...
            queueHandler.queueFunction(queueHandler.eventQueue, ui.message, "Erro no STT Groq.")
            return ""
    ...
    def _read_sse_stream(self, req: urllib.request.Request, is_gemini: bool) -> None:
        ...
        try:
            ...
            if full_response.strip():
             ...
             queueHandler.queueFunction(queueHandler.eventQueue, ui.message, "Erro de comunicação no streaming.")
    ...
    def _save_to_text_file(self, response: str) -> None:
        ...
        if save_hist and hist_file:
            try:
                prompt = self.history[-2]["content"] if len(self.history) >= 2 else "Transcrição indisponível"
                now = datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")
                
                stt_engine = self.config.get('general', 'stt_engine')
                if stt_engine == 'local':
                    stt_name = f"Whisper Local CLI (Modelo: {self.config.get('whisper_local', 'model')})"
                else:
                    stt_name = "Groq Whisper (Nuvem Rápida)"
                ...
                with open(hist_file, 'a', encoding='utf-8') as f:
                    ...
                    f.write(f"Motor STT: {stt_name}\n")
                    f.write(f"Provedor IA: {prov.upper()} | Modelo: {model}\n")
                    f.write(f"{'-'*60}\n")
                    f.write(f"Sua Pergunta / Contexto: \n{prompt}\n")
                    f.write(f"{'-'*60}\n")
                    f.write(f"Resposta da IA: \n{clean_response}\n")
            except Exception as e:
                logHandler.log.error(f"Erro ao escrever no arquivo de histórico: {e}")
