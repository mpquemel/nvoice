...
class NVoiceSettingsPanel(SettingsPanel):
    ...
    def makeSettings(self, settingsSizer: wx.Sizer) -> None:
        ...
        prov_sizer.Add(wx.StaticText(self, label="Provedor LLM:"), 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 5)
        ...
        # Motor de STT
        stt_box = wx.StaticBox(self, label="Reconhecimento de Voz (STT)")
        stt_box_sizer = wx.StaticBoxSizer(stt_box, wx.VERTICAL)
        
        stt_sizer = wx.BoxSizer(wx.HORIZONTAL)
        stt_sizer.Add(wx.StaticText(stt_box, label="Motor:"), 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 5)
        self.stt_cb = wx.ComboBox(stt_box, choices=["groq (Nuvem Rápida)", "local (Whisper CLI Offline)"], style=wx.CB_READONLY)
        stt_val = "local (Whisper CLI Offline)" if self.config.get('general', 'stt_engine') == 'local' else "groq (Nuvem Rápida)"
        self.stt_cb.SetValue(stt_val)
        stt_sizer.Add(self.stt_cb, 1, wx.ALL, 5)
        stt_box_sizer.Add(stt_sizer, 0, wx.EXPAND)
        
        # Modelo do Whisper Local
        wh_sizer = wx.BoxSizer(wx.HORIZONTAL)
        wh_sizer.Add(wx.StaticText(stt_box, label="Modelo Local (CLI):"), 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 5)
        ...
        for prov in ["groq", "openai", "gemini", "openrouter"]:
            ...
            sizer.Add(wx.StaticText(self, label=f"API Key {prov.capitalize()}:"), 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 5)
        ...
        ollama_sizer.Add(wx.StaticText(self, label="Ollama Endpoint:"), 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 5)
        ...
        hist_box = wx.StaticBox(self, label="Exportação de Histórico de Conversas")
        hist_sizer = wx.StaticBoxSizer(hist_box, wx.VERTICAL)
        
        self.save_hist_cb = wx.CheckBox(hist_box, label="Salvar conversas automaticamente em arquivo de texto (.txt)")
        ...
        path_sizer.Add(wx.StaticText(hist_box, label="Local do Arquivo:"), 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 5)
        ...
        self.btn_browse = wx.Button(hist_box, label="Procurar...")
        ...
        # Gestão de Modelos Dinâmicos
        model_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.btn_fetch = wx.Button(self, label="Buscar Modelos do Provedor Atual")
    ...
    def onBrowseHist(self, evt) -> None:
        with wx.FileDialog(self, "Selecione ou crie o arquivo de histórico", wildcard="Arquivos de texto (*.txt)|*.txt", style=wx.FD_SAVE) as fileDialog:
    ...
    def _fetch_models_thread(self, prov: str, api_key: str) -> None:
        ...
        try:
            ...
            logHandler.log.error(f"Erro buscando modelos: {e}")
            models = ["Erro ao buscar"]
